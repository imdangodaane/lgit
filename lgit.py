...
def lgit_get(file, cmd):
    '''
    This function get file information depend on cmd (command)
    '''
    try:
...
def get_all_files(dir_name=None):
    '''
    This function get all files in current directory and sub-directories
    '''
    files = []
...
def check_and_init():
    '''
    This function check .lgit directory was exist or not, if existed, check
    and print error, else Initialize a new one.
    '''
...
def add_file(file):
    '''
    This function make a copy of file content and save it in .lgit database (
    object directory), then update file information in the index file
    '''
    begin_dir = os.getcwd()
...
def lgit_add(files):
    '''
    This function handle for add multiple files or folder
    (include * and . sign)
    '''
    if len(files) > 0:
        for file in files:
...
def rm_file(file):
    '''
    This function check if file in tracked files, if yes, remove file and
    update index file without information of file, if not, print falal error
    '''
    try:
        f = open(file, 'r+')
    except FileNotFoundError:
    ...
    if len(file_prefix) == 2:
        ...
        file_prefix = ''
    if os.path.join(file_prefix, file) not in tracked_files:
        print("fatal: pathspec '%s' did not match any files" % file)
        sys.exit()
    lines = content.split('\n')
    for i in range(len(lines)):
        if len(lines[i]) > 0:
            if os.path.join(file_prefix, file) == lines[i].split()[-1]:
...
def lgit_rm(files):
    '''
    This function handle for remove multiple files
    '''
    if len(files) > 0:
        for file in files:
            if len(file) > 0:
...
def lgit_commit(message):
    '''
    This function includes: update index file and make commits, snapshots
    '''
...
def lgit_config(name):
    '''
    This function change author name
    '''
...
def lgit_ls_files():
    '''
    This function list all tracked files in current directory and
    sub-directories
    '''
    tracked_files = lgit_get(index_path, 'tracked_files')
    file_prefix = os.getcwd().split(lgit_dir + '/')
...
def lgit_log():
    '''
    This function show log information of commits
    '''
...
def find_lgit_dir():
    '''
    This function find '.lgit' directory in current directory and parents
    If find it, return the directory path contain '.lgit' dir,
    else return None
    '''
    count = 0
    ...
    while '.lgit' not in dirs:
        ...
        os.chdir(path)
        dirs = os.listdir()
        if path == '/home' and '.lgit' not in dirs or count > 100:
            os.chdir(begin_path)
            return None
        count += 1
    os.chdir(begin_path)
